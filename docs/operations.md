...
`notion-ui-viewer-kit` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for viewer scenarios with layout fixtures, stable geometry snapshots, and explicit failure cases.
...
The score is deterministic. The current threshold is 182. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 6.
