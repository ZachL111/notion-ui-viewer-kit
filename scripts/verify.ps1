...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-notion-ui-viewer-detail.ps1

lua tests/test_domain_review.lua

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-notion-ui-viewer-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-notion-ui-viewer-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-notion_ui_viewer_kit-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
